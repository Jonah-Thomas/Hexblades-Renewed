...
public class EarthHammer2 extends EarthHammer1 {
    ...
    public EarthHammer2(Properties props) {
        ...
        tooltipText = Component.translatable("tooltip.hexblades.earth_hammer2");
    }
    ...
    public void applyHexEffects(ItemStack stack, LivingEntity target, Player attacker, boolean awakened) {
        float power = 1.5F;
        if (awakened) {
            target.hurt(target.damageSources().anvil(attacker), COMMON.HammerED2.get().floatValue());
    ...
    protected void addShiftTooltip(ItemStack stack, List<Component> tooltip) {
        tooltip.add(Component.literal("Armor piercing damage: " + COMMON.HammerED2.get()));
        tooltip.add(Component.literal("Massive knockback, weakens enemies"));
    }
}
