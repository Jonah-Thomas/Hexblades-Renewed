...
public class FlameEffectPacket {
    ...
    public static void consume(FlameEffectPacket packet, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (world != null) {
                // TODO: Deity API may differ in Eidolon-Repraised — verify HexDeities.HEX_DEITY type
                ...
                Particles.create(EidolonParticles.FLAME_PARTICLE).setAlpha(1.0F, 0.0F).setScale(0.25F, 0.0F).setLifetime(20).randomOffset(0.5D, 0.0D).randomVelocity(0.0D, 0.375D).addVelocity(0.0D, 0.125D, 0.0D).setColor(r, g, b, r, g * 0.5F, b * 1.5F).setSpin(0.2F).repeat(world, x, y, z, 6);
            }
        });
