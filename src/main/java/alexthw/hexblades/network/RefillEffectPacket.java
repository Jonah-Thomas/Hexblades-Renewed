    ...
    public static void consume(RefillEffectPacket packet, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (world != null) {
                ...
                double y = pos.getY() + (size < 1 ? 1.1D : 0.85D);
                double z = pos.getZ() + 0.5D;
                float r = 0.25F;
                float g = 0.5F;
                float b = 1.0F;
                Particles.create(EidolonParticles.BUBBLE_PARTICLE).setAlpha(1.0F, 0.0F).setScale(0.075F).setLifetime(10).randomOffset(0.5D * size, 0.0D).randomVelocity(0.0D, 0.1D).addVelocity(0.0D, 0.05D, 0.0D).setColor(r, g, b, r, g * 0.5F, b * 1.5F).repeat(world, x, y, z, 10);
            }
        });
